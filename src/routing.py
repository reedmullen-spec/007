...
def resolve_ae_verbose(company_name: str, country: str, cfg: dict,
                       hubspot: HubSpotClient | None = None) -> tuple[str, bool]:
    """Returns (ae, from_hubspot). from_hubspot=True means the company's
    live HubSpot owner decided it — a confident single-AE answer."""
    routing = cfg["routing"]
    ...
    return _geographic_ae(company_name, country, cfg), False


def resolve_ae(company_name: str, country: str, cfg: dict,
               hubspot: HubSpotClient | None = None) -> str:
    return resolve_ae_verbose(company_name, country, cfg, hubspot)[0]


def us_state_ae(state: str, cfg: dict) -> str:
    """State -> White Cap-team AE, per the Aug 2026 territory map."""
    for ae, states in cfg["routing"].get("us_state_ae", {}).items():
        if state and state.upper() in [s.upper() for s in states]:
            return ae
    return ""


def _geographic_ae(company_name: str, country: str, cfg: dict) -> str:
    routing = cfg["routing"]
    ...
    extra = routing.get("country_ae_map_extra", {})
    if country in extra:
