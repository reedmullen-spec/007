...
def main() -> int:
    ...
    def card_targets(p, ae, from_hs=False):
        ...
        if p.source == "AUSTENDER":
            ...
            # UK: if HubSpot company ownership decided the AE, tag them
            # alone; otherwise Aled and Lisa split it — tag both.
            if from_hs:
                return [cfg["slack"]["tender_channel_id"]], [ae_slack.get(ae, "")]
            names = region_mentions.get("uk", [])
            mentions = [ae_slack.get(n, "") for n in names] or [ae_slack.get(ae, "")]
            return [cfg["slack"]["tender_channel_id"]], mentions
        else:
            return [cfg["slack"]["tender_channel_id"]], [ae_slack.get(ae, "")]
    ...
    for p in candidates:
        ...
        ae, from_hubspot = resolve_ae_verbose(p.buyer, p.country, cfg, hubspot)
        lines = [
