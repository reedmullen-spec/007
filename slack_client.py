...
"""
from __future__ import annotations"""Slack client for 007.
...
class SlackClient:
        ...
        while True:
            ...
            for msg in data.get("messages", []):
                ...
                if meta is None:
                    continue
                if meta.get("wp"):
                    # Weekly parent: history doesn't include thread replies,
                    # so open the thread and scan the cards inside it.
                    yield from self._scan_thread(channel, msg["ts"], approver)
                    continue
                hit = self._check_approval(msg, approver)
                if hit:
                    yield msg["ts"], hit
    ...
    def _scan_thread(self, channel: str, parent_ts: str, approver: str):
        cursor = None
        while True:
            params = {"channel": channel, "ts": parent_ts, "limit": 200}
            if cursor:
                params["cursor"] = cursor
            data = self._call("conversations.replies", params=params)
            for msg in data.get("messages", []):
    ...
    def _check_approval(self, msg: dict, approver: str) -> dict | None:
        meta = self._extract_meta(msg)
        if meta is None or meta.get("wp"):
            return None
